    ...
    class Problems_DynamicProgramming_TextWrap
    {
        ...
        public class Solution5
        {
            ...
            {           
                ...
                for (int x = i; x <= j && totalWidth <= PageWidth; x++)
                {
                    //totalWidth += this.words[x].Length;// wordsLen[x];            //<-- this costs too much
                    totalWidth += wordsLen[x];
                    if (x != i)
                    {
                        totalWidth += 1;    //Add space if not first word
            ...
            Dictionary<string, int> getWidthHash = null; //new Dictionary<string, int>();
            public int GetWidth(int i, int j)
            {
                string key = i.ToString() + "_" + j.ToString();
                if (getWidthHash.ContainsKey(key))
                    return getWidthHash[key];

                int width = 0;
                if (i == j)
                {
                    width = this.words[i].Length;
                }
                else
                {
                    width = this.words[i].Length + 1 + GetWidth(i+1, j);
                }

                getWidthHash[key] = width;
                return width;
            }
            ...
            private void CreateEdges(Node n, int maxJ)
            {
                ...
                else
                {
                    ...
                    {
                        //Check to see if the badness of this suffix is infinity (double.MaxValue)
                        if (Badness(i,j) < double.MaxValue)// matrix[i, j] < double.MaxValue)
                        {
                            //badness is not infinity, so request a node for this (i,j) pair
                            Node newN = newNode(i, j);
                            ...
                            if (!found)
                            {
                                //Edge does not already exist, so add an edge to this node
                                //And then recursivly create all the edges for the new node
                                n.Edges.Add(new Edge(newN, Badness(i, j)));//   matrix[i, j]));
                                CreateEdges(newN, maxJ);
                            }
                        }
                    }
            ...
            bool precalcMatrix = false;
            public void Run(string inputWords)
            {
                getWidthHash = new Dictionary<string, int>();
                getWordsHash = new Dictionary<string, string>();
                badnessHash = new Dictionary<string, double>();
                nodeHash = new Dictionary<string, Node>();
                ...
                ////////////////////////////////////////////////////////////////////////////////////////
                //Optimizaiton - Convert segmented words to just integer lengths to reduce reduntant length calculations
                //Convert words to just length
                this.wordsLen = new int[this.words.Length];
                for (int x = 0; x < this.words.Length; x++)
                {
                ...
                ////////////////////////////////////////////////////////////////////////////////////////
                //Optimization - Create matrix of 'badness' values
                //All possible (i,j) combinations can be cached in the matrix
                //which will be faster than a hash table since there are no collissions

                //Initialize the matrix with 'infinity' values (double.MaxValue) since we will only
                //be calculating the badness values for items that fit in the page width
                matrix = new double[wordsLen.Length, wordsLen.Length];
                for (int i = 0; i < wordsLen.Length; i++)
                {
                    for (int j = 0; j < wordsLen.Length; j++)
                ...
                //Perform the actual badness calculations and store in the matrix
                precalcMatrix = true;
                DateTime bStart = DateTime.Now;
                for (int i = 0; i < wordsLen.Length; i++)
                {
                ...
                ////////////////////////////////////////////////////////////////////////////////////////
                //Create the head and end nodes for use in Dijstra algorithm for shortest path
                head = new Node(-1, -1, "<HEAD>");
                end = new Node(-1, -1, "<END>");
                ...
                for (int i = 0; i < 1; i++)
                {
                    for (int j = i; j < this.words.Length; j++)//  wordsLen.Length; j++)
                    {
                        if (Badness(i,j) < double.MaxValue)//matrix[i, j] < double.MaxValue)
                        {
                            head.Edges.Add(new Edge(new Node(i, j), Badness(i, j)));// matrix[i, j]));
                        }
                    }
                }
                ...
                foreach (Edge e in head.Edges)
                {
                    CreateEdges(e.Node, this.words.Length-1);// wordsLen.Length - 1);
                }
                DateTime endTime = DateTime.Now;
                Console.WriteLine("CreateEdges: " + (endTime - startTime).TotalSeconds);
